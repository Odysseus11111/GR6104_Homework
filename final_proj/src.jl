...
function runtime_benchmark(prices::Matrix{Float64}, eta_list::Vector{Float64})
    n, m = size(prices)
    T = n - 1
    results = []

    for eta in eta_list
        println("Running eta = ", eta)

        # Number of grid points
        grid = simplex_grid_md(m, eta)
        num_grid_points = length(grid)

        # Benchmark UP runtime
        up_runtime = @elapsed begin
            up_wealth, b_weights, S_grid = uni_port_func(prices, eta)
        end

        up_final_wealth = up_wealth[end]

        # Run grid BCRP with the same eta
        bcrp_runtime = @elapsed begin
            best_b, bcrp_final_wealth, best_wealth_set = bcrp_func_md(prices, eta)
        end

        # Compute log regret
        
        total_regret, average_regret = log_regret(
            bcrp_final_wealth,
            up_final_wealth,
            T
        )
        ...
        push!(results, (
            ...
            up_final_wealth = up_final_wealth,
            bcrp_final_wealth = bcrp_final_wealth,
            log_regret = total_regret,
            average_log_regret = average_regret,
            up_runtime = up_runtime,
            bcrp_runtime = bcrp_runtime
        ))
    end
